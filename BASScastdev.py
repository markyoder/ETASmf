...
class BASScast(object):
	...
	def Z2str(self):
		...
		return outstr
	#
	def contsToPlotLists(self, cset=None, returnlevels=False):
		...
		levels=[]	# list of levels, synghed to outlist.
		levelindex=-1
		#
		for i in xrange(len(cs)):
			# level-level:
			...
			for trace in cs[i].get_paths():
				...
				# this is not the case. find the 2xinner (aka, outer) polys within as per comments above.
				#
				newpolylist=self.innerouterpolys(newpoly)	# so now, we've broken up this /trace/ into multiple /traces/, but the levels
																		# situtation has not changed...
				#newpolylist=[newpoly]
				# return the full list and interpret i=0 as the outer ring; i>0 as the inner boundaries.
				#
				#outlist[-1]+=[newpoly]
				for ply in newpolylist:
					#outlist[-1]+=[newpoly]	# note that now, there are multiple entries per "level", so we have to index each entry.
					#outlist[-1]+=[[countindex, newpoly]]
					#
					print "plyLen: %d" % len(ply)
					outlist+=[[]]
					outlist[-1]+=[ply]
					#outlist+=[[ply]]
					levels+=[levelindex]
				...
				'''
					#
		print "lens1: ", len(outlist), len(levels)
		#for i in xrange(len(outlist)):
		#	print levels[i], outlist[i]
		#return outlist
			
		if returnlevels==False:
			return outlist
		if returnlevels==True:
			return [levels, outlist]	# and now, this whole thing is getting complicated worthy of a new class (if it gets any worse).
	...
	def plotPolyList(self, polys=None, markerstr='.-', fignum=3, pllevels=None):
		# this plots in one color?? the contour lines. i think it is primarily diagnostic.
	...
	def writeKMLfile(self, cset=None, fout='conts.kml', colorbarname='scale.png', equakes=None):
		...
		kmlstr=self.KMLstrFromConts(cset, openfile=True, closefile=False)
		if equakes !=None:
	...
	def KMLstrFromConts(self, cset=None, colorbarname='scale.png', openfile=True, closefile=True, warnings=None):
		...
		cs=cset.collections
		polys0 = self.contsToPlotLists(cset=cset, returnlevels=True)	# this function fixes multi-closed polys.
																# use for actual kml contours. also, return list of levels for kml processing.
		...
		polys = polys0[1]
		levels = polys0[0]
		print "lens: ", len(cs), len(polys), len(levels), len(set(levels))
		#
		#resolution = 5
		...
		# i think this will still give the proper number of levels. otherwise, use len(set(levels)).
		for i in range(0,len(cs)):
		#for i in range(0, len(set(levels))):
		...
		for i in xrange(startindex, len(polys)):
			...
			warningindex=int(float(level)/float(resolution))
